...
export default function EditBanner(){
        ...
        if(id) {
            ...
            .then(({data}) => {
                ...
                setBanner(prevBanner => ({
                    ...
                    items: itemIds,
                    imageUrl: `${import.meta.env.VITE_API_BASE_URL}/storage/${data.banner_image}`, // Set imageUrl here
                }))
            })
        }
    }, [])
    ...
    const onSubmit = (e) => {
        ...
        if(id){
            console.log(formData);
            axiosClient.post(`/banners/${banner.id}`, formData, {
    ...
    const handleFileChange = (e) => {
        ...
        if (selectedFile) {
            ...
            setBanner({ ...banner, banner_image: selectedFile, imageUrl: imageUrl });
        } else {
                ...
                {categories.length > 1 ? (
                    categories.filter(category => category.id !== 1).map((category) => (
                        <label key={category.id}>
                            <input type="checkbox"
                            checked={banner.categories.includes(category.id)}
                            onChange={() => {
                                setBanner((prevBanner) => {
...
<tr key={item.id}>
    <td>
        <input
            type="checkbox"
            checked={banner.items.includes(item.id)}
            onChange={() => {
                setBanner((prevBanner) => {
                    const updatedItemIds = prevBanner.items.includes(item.id)
                        ? prevBanner.items.filter((id) => id !== item.id)
                        : [...prevBanner.items, item.id];
